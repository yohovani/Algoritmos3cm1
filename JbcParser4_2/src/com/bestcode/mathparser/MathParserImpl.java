...
class MathParserImpl implements IMathParser {
  ...
  MathParserImpl() {
    ...
    if (translate==null){
      translate = new StrUtil(loadBundle());
      if (translate!=null){
        translators.put(locale, translate);
      }
    }
  ...
  private ResourceBundle loadBundle(){
    ...
    try{
      ...
      rb = ResourceBundle.getBundle(this.getClass().getPackage().getName()+".mathparser", locale, this.getClass().getClassLoader());
    }
    catch(Exception ex){
    }
    if(rb==null){
  ...
  public void setLocale(Locale l){
    ...
    if (t!=null){ //if already loaded, use existing...
      ...
      translate = new StrUtil(loadBundle());
      if (translate!=null){
        translators.put(l, translate); //save for future re-use.
      }
    }
  }
  ...
  public double getVariable(String varName) throws Exception {
    ...
    if (aVariable==null){
      throw new Exception(translate.getMessage("VarNtExst", varName));
    }
  ...
  public void setVariable(String varName, double newVal) throws Exception {
    ...
    if (existing!=null){
      if(existing.isConstant){
        throw new Exception(translate.getMessage("CnstVal", varName));
      }
      existing.value = newVal;
    }
    else {
      if (!isValidName(upcName)){
        throw new Exception(translate.getMessage("NtVarNm", varName));
      }
      //create the variable:
  ...
  public void parse() throws Exception, ParserException{
    ...
    */
    if (m_Expression==null || ! (m_Expression.length() > 0)){
      m_Node = null;
      throw new Exception(translate.getMessage("ExpEmpty"));
    }
    ...
    if (brackets>-1 && brackets<formula.length()){
			throw new ParserException(translate.getMessage("BrcktMis", formula.toString(),
					String.valueOf(brackets)), formula.substring(brackets, formula.length()).toString(),
					formula.toString());
    }
    else
  	if(brackets==formula.length()){
    	throw new ParserException(translate.getMessage("MisBrckt", formula.toString()), formula.toString(), formula.toString());
  	}
    
    if ((m_Node = createParseTree(formula)) == null){
    	throw new ParserException(translate.getMessage("ExpNtVld", formula.toString(), formula.toString()), formula.toString(), formula.toString());
    }
  ...
  public void createConstant(String varName, double newVal) throws Exception {
    ...
    if (existing!=null){
      ...
      throw new Exception(translate.getMessage("VarExt", varName));
    }
    else {
      if (!isValidName(upcName)){
        throw new Exception(translate.getMessage("NtVarNm", varName));
  ...
  throws Exception {
    ...
    if (!isValidName(upcName)) { //must contain uppercase letters only
      throw new Exception(translate.getMessage("NtFncNm", newFuncName));
    }

    if (isFunction(upcName)) {
      throw new Exception(translate.getMessage("FncExst", newFuncName));
    }
  ...
  */
  ...
  //------------------------------------------------------------------------------
  protected static boolean isValidDouble(FastStr formula) {
    //Double.valueOf(formula) thinks "020" is a valid double.
    if (formula.length()>1 && (formula.charAt(0)=='0' && formula.indexOf('.')!=1)){
      return false;
    }
  ...
  protected Node createParseTree(FastStr expToParse) throws ParserException {
    ...
    if (! (lastOper>0) ) //if it is 0 then it is a unary operation which is a one param function
    {
      ...
      if ( isOneParamFunc(formula, param, lastOper) )
      {
        if ((leftNode = createParseTree(param.param)) == null ){
             throw new ParserException(translate.getMessage("ExpNtVld", param.param, formula), param.param.toString(), formula.toString());
             //return null;
        }
    ...
    if (isTwoParamFunc(formula, param, lastOper) )
    {
      if ((leftNode = createParseTree(param.paramLeft)) == null){
				throw new ParserException(translate.getMessage("ExpNtVld", param.paramLeft, formula),
						param.paramLeft.toString(), formula.toString());
      }
      if ((rightNode = createParseTree(param.paramRight)) == null ){
				throw new ParserException(translate.getMessage("ExpNtVld", param.paramRight, formula),
    ...
    if (isNParamFunc(formula, Nparam, lastOper) )
    {
      ...
      if (params==null){
				throw new ParserException(translate.getMessage("InvNPrm", formula), formula.toString(),
						formula.toString());
      }
      ...
      for (int i=0; i<nParam; i++){
        if ( (leftNode = createParseTree(params[i]) )== null){
					throw new ParserException(translate.getMessage("ExpNtVld", params[i], formula),
  ...
  //------------------------------------------------------------------------------
  private static int findLastOper(FastStr formula) { //returns -1 if it cannot find anything
...
class __min extends TwoParamFunc {
  public double run(IParameter[] p){
    double x__ = p[0].getValue();
    double y__ = p[1].getValue();
    if (x__ < y__)
      return x__;
    else
      return y__;
...
class __max extends TwoParamFunc {
  public double run(IParameter[] p){
    double x__ = p[0].getValue();
    double y__ = p[1].getValue();
    if (x__ > y__)
